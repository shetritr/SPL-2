...
public class Promise<T>{
	...
	private Queue <callback> queueOfCallBack = new ConcurrentLinkedQueue<callback>();
	/**
	 *
	 ...
	 */
	public synchronized void resolve(T value){ // Becuse i dont want the one thread add to the Queue and one thread done to callback
		if(isResolved())
			throw new IllegalStateException();
		isResolved = true;
		ResolvedValue = value;
		while (!queueOfCallBack.isEmpty()){
			queueOfCallBack.poll().call();//delete the old callback from the queueOfCallBack
		}
	...
	public void subscribe(callback callback) {
		...
		{
			if(!queueOfCallBack.contains(callback))
				queueOfCallBack.add(callback);
		}
	}
}
